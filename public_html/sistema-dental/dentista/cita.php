...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($nuevoEstado, ['pendiente', 'confirmada', 'cancelada', 'completada'], true)) {
        $stmt = $pdo->prepare("UPDATE citas SET estado = ?, dentista_id = ? WHERE id = ?");
        $stmt->execute([$nuevoEstado, $usuario['id'], $citaId]);
    }
}

$stmt = $pdo->prepare("
    SELECT c.*, u.nombre AS paciente_nombre, u.email AS paciente_email, u.telefono AS paciente_telefono
    FROM citas c
...
<title>Cita — Clínica Dental</title>
...
<div class="app-shell">
    ...
    <main class="main">
        <header>
            <div>
                <a href="agenda.php" style="font-size:13px; color:var(--text-muted); text-decoration:none;">&larr; Volver a la agenda</a>
                <h1 style="margin-top:6px;">Cita de <?= htmlspecialchars($cita['paciente_nombre']) ?></h1>
                <p><?= date('d/m/Y', strtotime($cita['fecha'])) ?> · <?= date('H:i', strtotime($cita['hora'])) ?> h</p>
            </div>
            <span class="pill pill-<?= $cita['estado'] ?>"><?= ucfirst($cita['estado']) ?></span>
        </header>

        <div class="panel">
            <h3>Detalles</h3>
            <table>
                <tr><th style="width:160px;">Paciente</th><td><a href="historia_clinica.php?id=<?= $cita['paciente_id'] ?>"><?= htmlspecialchars($cita['paciente_nombre']) ?></a></td></tr>
                <tr><th>Correo</th><td><?= htmlspecialchars($cita['paciente_email']) ?></td></tr>
                <tr><th>Teléfono</th><td><?= htmlspecialchars($cita['paciente_telefono'] ?: '—') ?></td></tr>
                <tr><th>Motivo</th><td><?= htmlspecialchars($cita['motivo'] ?: '—') ?></td></tr>
            </table>
        </div>
        ...
        <div class="panel">
            ...
            <form method="POST" style="display:flex; gap:10px; flex-wrap:wrap;">
                <button type="submit" name="estado" value="confirmada" class="btn" style="max-width:160px; background:#1F7A56;">Confirmar</button>
                <button type="submit" name="estado" value="completada" class="btn" style="max-width:160px; background:#2E4C82;">Marcar completada</button>
                <button type="submit" name="estado" value="cancelada" class="btn" style="max-width:160px; background:#B7332A;">Cancelar</button>
            </form>
        </div>
    </main>

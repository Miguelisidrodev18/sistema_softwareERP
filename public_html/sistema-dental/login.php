...
if ($usuario = usuario_actual()) {
    header('Location: ' . ($usuario['rol'] === 'dentista' ? 'dentista/dashboard.php' : 'paciente/dashboard.php'));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email === '' || $clave === '') {
        ...
        $stmt = $pdo->prepare('SELECT * FROM usuarios WHERE email = ? AND rol = ? LIMIT 1');
        ...
        if ($user && password_verify($clave, $user['password_hash'])) {
            ...
            header('Location: ' . ($rol === 'dentista' ? 'dentista/dashboard.php' : 'paciente/dashboard.php'));
            exit;
        } else {
            $error = 'Correo, contraseña o tipo de cuenta incorrectos.';
        }
    }
}
?>
...
/* Estilos específicos de esta página (login) */
.login-scene {
    min-height: 100vh;
    display: grid;
...
.scan-line-wrap svg { width: 100%; height: 100%; opacity: 0.9; }
...
.scan-line {
    ...
    box-shadow: 0 0 14px 2px rgba(191,237,226,0.7);
}
...
<div class="login-scene">
    ...
    <div class="login-intro">
        ...
        <h1>Cuidado dental, <span class="accent">organizado con precisión.</span></h1>
        <p>Un solo lugar para que el equipo clínico gestione tratamientos y para que cada paciente siga su historial y sus citas.</p>

        <div class="scan-line-wrap">
            <svg viewBox="0 0 260 130" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M130 8c-16 0-29 10.5-35 23.5C89.5 43 88 55 89 66c1.7 17 6.5 34.4 12.3 49.4 2 5.2 10.4 6.5 13-.6 3.2-7.1 5.8-18.2 8.5-26 1.3-3.9 3.9-6.5 7.2-6.5s5.9 2.6 7.2 6.5c2.7 7.8 5.3 18.9 8.5 26 2.6 7.1 11 5.8 13 .6C164.5 100.4 169.3 83 171 66c1-11 -.5-23-6-34.5C159 18.5 146 8 130 8Z" stroke="#BFEDE2" stroke-width="1.3" opacity="0.55"/>
            </svg>
    ...
    <div class="login-card-holder">
        <div class="login-card">
            ...
            <p class="sub">Elige tu tipo de cuenta e ingresa tus datos.</p>
            ...
            <form method="POST" action="login.php">
                <div class="role-toggle">
                    <input type="radio" name="rol" id="rol-paciente" value="paciente" checked>
                    <label for="rol-paciente">Paciente</label>
                    <input type="radio" name="rol" id="rol-dentista" value="dentista">
                    <label for="rol-dentista">Dentista</label>
                </div>
                ...
                <div class="form-foot">
                    ¿Eres paciente nuevo? <a href="register.php">Crea tu cuenta</a>
                </div>
            </form>
        </div>
    </div>
...
<script>
// Parallax 3D suave para la tarjeta de login, siguiendo el mouse.
(function () {
    const card = document.querySelector('.login-card');
    const holder = document.querySelector('.login-card-holder');

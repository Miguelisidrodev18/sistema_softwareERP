...
<title>Mi historial — Clínica Dental</title>
...
<div class="app-shell">
    ...
    <aside class="sidebar">
        ...
        <nav>
            ...
            <a href="historial.php" class="active"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 3"/></svg><span>Historial</span></a>
    ...
    <main class="main">
        <header>
            <div>
                ...
                <p>Tratamientos y notas registradas por tu dentista.</p>
            </div>
        </header>
        ...
        <div class="panel">
            <?php if (count($tratamientos) === 0): ?>
                <div class="empty-state">
                    Todavía no tienes tratamientos registrados. Aparecerán aquí apenas tu dentista los agregue.
                </div>
            <?php else: ?>
                <?php foreach ($tratamientos as $t): ?>
                    <div style="padding:16px 0; border-bottom:1px solid var(--border-soft);">
                        <div style="display:flex; justify-content:space-between; align-items:center;">
                            <strong style="color:var(--color-primary); font-size:15px;"><?= htmlspecialchars($t['descripcion']) ?></strong>
                            <span class="eyebrow"><?= date('d/m/Y', strtotime($t['fecha'])) ?></span>
                        </div>
                        <?php if ($t['dentista_nombre']): ?>
                            <div style="font-size:13px; color:var(--text-muted); margin-top:4px;">Atendido por <?= htmlspecialchars($t['dentista_nombre']) ?></div>
                        <?php endif; ?>
                        <?php if ($t['notas']): ?>
                            <p style="font-size:14px; margin-top:8px; color:var(--text-main);"><?= nl2br(htmlspecialchars($t['notas'])) ?></p>
                        <?php endif; ?>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>

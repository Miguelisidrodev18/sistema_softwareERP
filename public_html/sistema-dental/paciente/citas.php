...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($fecha === '' || $hora === '') {
        $error = 'Elige una fecha y hora para tu cita.';
    } else {
        $stmt = $pdo->prepare("INSERT INTO citas (paciente_id, fecha, hora, motivo, estado) VALUES (?, ?, ?, ?, 'pendiente')");
...
<div class="app-shell">
    ...
    <aside class="sidebar">
        ...
        <nav>
            <a href="dashboard.php"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M3 11.5 12 4l9 7.5"/><path d="M5 10v9a1 1 0 0 0 1 1h4v-6h4v6h4a1 1 0 0 0 1-1v-9"/></svg><span>Mi panel</span></a>
            <a href="citas.php" class="active"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3.5" y="5" width="17" height="16" rx="2"/><path d="M8 3v4M16 3v4M3.5 10h17"/></svg><span>Mis citas</span></a>
    ...
    <main class="main">
        <header>
            <div>
                <h1>Mis citas</h1>
                <p>Solicita una nueva cita o revisa el estado de las anteriores.</p>
            </div>
        </header>
        ...
        <div class="panel">
            ...
            <?php if ($ok): ?>
                <div class="alert alert-ok">Tu cita fue solicitada. Quedará como <strong>pendiente</strong> hasta que la clínica la confirme.</div>
            <?php endif; ?>
            <?php if ($error): ?>
                <div class="alert alert-error"><?= htmlspecialchars($error) ?></div>
            <?php endif; ?>
            <form method="POST" action="citas.php" class="inline-form">
                <div class="two-col">
                    <div class="field">
                ...
                <div class="field">
                    ...
                    <input type="text" id="motivo" name="motivo" placeholder="Ej: limpieza, dolor, revisión...">
                </div>
                <button type="submit" class="btn" style="max-width:220px;">Solicitar cita</button>
            </form>
        </div>
        ...
        <div class="panel">
            ...
            <?php if (count($citas) === 0): ?>
                <div class="empty-state">Todavía no tienes citas registradas.</div>
            <?php else: ?>
            <table>
                <thead>
                    <tr><th>Fecha</th><th>Hora</th><th>Motivo</th><th>Estado</th></tr>
                </thead>
                <tbody>
                <?php foreach ($citas as $c): ?>
                    <tr>
                        <td><?= date('d/m/Y', strtotime($c['fecha'])) ?></td>
                        <td><?= date('H:i', strtotime($c['hora'])) ?> h</td>
                        <td><?= htmlspecialchars($c['motivo'] ?: '—') ?></td>
                        <td><span class="pill pill-<?= $c['estado'] ?>"><?= ucfirst($c['estado']) ?></span></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>

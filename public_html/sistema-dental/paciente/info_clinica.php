...
$info = $pdo->query("SELECT * FROM clinica_info LIMIT 1")->fetch();
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<title>La clínica — Clínica Dental</title>
...
<div class="app-shell">
    ...
    <aside class="sidebar">
        ...
        <nav>
            ...
            <a href="info_clinica.php" class="active"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 21s-7-6.2-7-11a7 7 0 0 1 14 0c0 4.8-7 11-7 11z"/><circle cx="12" cy="10" r="2.5"/></svg><span>La clínica</span></a>
        </nav>
    ...
    <main class="main">
        <header>
            <div>
                ...
                <p>Dirección, contacto y horario de atención.</p>
            </div>
        </header>

        <div class="cards-grid">
            <div class="stat-card">
                <div class="label" style="margin-bottom:6px;">Dirección</div>
                <div style="font-size:15px; color:var(--color-primary); font-weight:600;"><?= htmlspecialchars($info['direccion'] ?? '—') ?></div>
            </div>
            <div class="stat-card">
                <div class="label" style="margin-bottom:6px;">Teléfono</div>
                <div style="font-size:15px; color:var(--color-primary); font-weight:600;"><?= htmlspecialchars($info['telefono'] ?? '—') ?></div>
            </div>
            <div class="stat-card">
                <div class="label" style="margin-bottom:6px;">Correo</div>
                <div style="font-size:15px; color:var(--color-primary); font-weight:600;"><?= htmlspecialchars($info['email'] ?? '—') ?></div>
            </div>
        </div>

        <div class="panel">
            <h3>Horario de atención</h3>
            <p style="font-size:14px; color:var(--text-muted);"><?= htmlspecialchars($info['horario'] ?? 'Por confirmar') ?></p>
        </div>
    </main>

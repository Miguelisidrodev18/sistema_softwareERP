<x-app-layout>
    ...
    <x-slot name="header">
        <div class="flex items-center gap-2 text-sm">
            ...
            <a href="{{ route('eventos.show', $evento) }}" class="text-slate-600 hover:text-slate-400 transition-colors font-mono">{{ $evento->nombre }}</a>
            <span class="text-slate-700">/</span>
            <span class="text-white font-semibold">Editar lead</span>
        </div>
    </x-slot>
    ...
    <div class="max-w-3xl">
        ...
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-slate-500 mt-0.5">{{ $lead->nombres }} · {{ $evento->nombre }}</p>
            </div>
        ...
        <form method="POST" action="{{ route('eventos.leads.update', [$evento, $lead]) }}">
            @csrf
            @method('PUT')

            @include('eventos._lead_form')
